...
public partial class ThermalVisionDialog : UserControl, IEffectDialog
{
    public event EventHandler<EffectEventArgs>? ApplyRequested;
    public event EventHandler<EffectEventArgs>? PreviewRequested;
    public event EventHandler? CancelRequested;

    public ThermalVisionDialog()
    {
    ...
    private ThermalVisionImageEffect CreateEffect()
    {
        return new ThermalVisionImageEffect
        {
            Levels = (int)Math.Round(GetValue("LevelsSlider", 8d)),
            Contrast = GetValue("ContrastSlider", 135d),
            Glow = GetValue("GlowSlider", 28d),
